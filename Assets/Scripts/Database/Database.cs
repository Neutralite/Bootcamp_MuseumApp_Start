...
public static class Database
{
    ...
    public static UserRating GetUserAttractionRating(string attractionID)
    {
        ...
        var results = connection.Query<UserRating>($@"SELECT * FROM {nameof(UserRating)} WHERE {nameof(UserRating.AttractionID)} = '{attractionID}'AND
                      {nameof(UserRating.Username)} = '{username}'");

        Debug.Assert(results.Count <= 1, $"{username} has multiple rating for the same attraction.");
